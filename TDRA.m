    ...
    for iIter = 1 : Rmax
        ...
        switch nHeuristic
            ...
            case 4
                ...
                fs_prime = f(aafDistances, s_prime, u); 
            case 5
                s_prime = apply_heuristic_5_Origin_Destination(s, C0, aafDistances, u); 
                fs_prime = f(aafDistances, s_prime, u); 
...
% Heuristic 5
function[ solnNew ] = apply_heuristic_5_Origin_Destination(solnIn, C0, aafDistances, k)
% The origin destination heuristic will take in a current solution and
% calculate the distance. It will create a list of all customers served at
% rendezvous locations between truck & UAV (opposite of greedy assignment),
% select a customer i randomly from the list and remove it from its current
% route. It will then look at every potential position in truck route to
...
%
    ...
    % Local Variable
    %   bLeavingOrReturning     boolean array; 1 if customer is rendevous
    %                           location, 0 otherwise
    % 

    % Calculate distance of input solution
    fSolnIn = f(aafDistances, solnIn, k); 
